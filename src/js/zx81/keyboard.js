let zx81_keyboard = (function(options) {
  const keyStates = [];
  ...
  const keyCodes = {
    ...
    // row 8 doesn't exist, but is used to press two ZX keys at once, SHIFT and one other
    8: {
      row: 8,
      mask: 0x01
    },
  ...
  (function ctor() {
    ...
    emf.input.setAutoRepeat(false);
  })();

  function setState(json) {
    json.keys.forEach((row) => {
      keyStates[row] = json.keys[row];
    });
  }
