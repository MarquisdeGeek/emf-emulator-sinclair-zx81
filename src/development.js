...
function SGXPrepare_OS() {
  ...
  // FILE: ui/web/prepare.js
  // PURPOSE: Code to be run inside the SGX prepare function, at the start of the emulator
  // process, before anything has loaded or been run.

  gStateVars.debugemf.addCommand('load', 'load <eaas_id> ; load snapshot from EaaS', function(cmd) {
    let importer = new emf.importer();
    let id = cmd[1];
    let filename = emf.eaas.getResourceURL(id);

    importer.byURL(filename)
      .then(function(data) {
        let string = new TextDecoder("utf-8").decode(data);
        let json = JSON.parse(string);
        let sw = emf.eaas.getResourceURL(json.resource);

        filename = json.filename;

        console.log(json);

        return importer.byURL(sw);
      })
      .then(function(data) {
        importMachineData(gStateVars.machine, filename, data);
      })
      .catch(function(ex) {
        console.error(`Could not load ${id}... ${ex.message}`);
      })
    return [`Attempting to load ${id}...`];
  });
}
